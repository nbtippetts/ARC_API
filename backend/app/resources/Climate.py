...
from app.models import RoomModel, IPModel, ClimateLiveDataModel, ClimateScheduleModel, ClimateIntervalModel, ClimateModel, ClimateDayNightModel, ClimateScheduleLogModel, ClimateLogModel
...
class ClimateParametersList(Resource):
	...
	def get(self):
		climate = ClimateModel.query.all()
		return climate, 200
...
sht_climate_parser.add_argument('humidity', type=float, help='Invalid Humidity')
sht_climate_parser.add_argument('temperature', type=float, help='Invalid Temperature')
...
class SHTClimateLog(Resource):
	def get(self):
		...
		vpd = round(vpd, 2)
		fahrenheit = (args['temperature'] * 9/5) + 32
		climate_log = ClimateLogModel(humidity=args['humidity'], temperature=fahrenheit, vpd=vpd, timestamp=log_timestamp, IP=ips)
		db.session.add(climate_log)
		db.session.commit()
		return 'SUCCESS', 200
...
api.add_resource(ClimateParametersList, '/climate_parameters')
